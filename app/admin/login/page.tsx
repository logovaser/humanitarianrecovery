...
export default function AdminLoginPage() {
  ...
  const hasPasswordAuth = Boolean(process.env.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD_HASH);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-surface px-6 py-12">
      {configured ? (
        <LoginForm hasPasswordAuth={hasPasswordAuth} hasGoogleAuth={hasGoogleAuth} />
      ) : (
        <div className="max-w-md rounded-2xl bg-white p-8 text-center shadow-lg">
          ...
          <p className="mt-3 text-sm text-ink/70">
            ...
            <code className="text-xs">ADMIN_EMAILS</code>, and either password or Google OAuth
            credentials in your environment. See <code className="text-xs">.env.example</code>.
          </p>
        </div>
      )}
    </div>
